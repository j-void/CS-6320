...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  im_left = torch.randint(0, 2, (H, W)) 
  im_right = im_left.detach().clone()
  hs, he, ws, we = H//2 - block_size[0]//2, H//2 + block_size[0]//2, W//2 - block_size[1]//2 , W//2 + block_size[1]//2
  block = im_right[hs:he+1, ws:we+1]
  im_right[hs:he+1, ws-disparity:we-disparity+1] = block
  patch = im_right[hs:he+1, we-disparity+1:we+1]
  im_right[hs:he+1, we-disparity+1:we+1] = torch.randint(0, 2, (patch.shape))

  im_left = torch.stack([im_left]*C, dim=2)
  im_right = torch.stack([im_right]*C, dim=2)
