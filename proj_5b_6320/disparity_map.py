...
def calculate_disparity_map(left_img: torch.Tensor,
  ...
  for h in range(0, H-block_size+1, 1):
    for w in range(0, W-block_size+1, 1):
      left_window = left_img[h:h+block_size, w:w+block_size, :]
      se = []
      for s in range(w, max(0, w-max_search_bound+1), -1):
        right_window = right_img[h:h+block_size, s:s+block_size, :]
        se.append(sim_measure_function(left_window, right_window))
      disparity_map[h,w] = float(np.argmin(np.array(se))) if len(se) > 0 else 0
  ############################################################################
  # Student code end
  ############################################################################
...
def calculate_cost_volume(left_img: torch.Tensor,
  ...
  for h in range(H-block_size+1):
    for w in range(W-block_size+1):
      left_window = left_img[h:h+block_size, w:w+block_size, :]
      se = np.full((max_disparity), 255.)
      for idx, s in enumerate((range(w, max(0, w-max_disparity), -1))):
        right_window = right_img[h:h+block_size, s:s+block_size, :]
        se[idx] = sim_measure_function(left_window, right_window)
      cost_volume[h,w] = torch.tensor(se).float()
