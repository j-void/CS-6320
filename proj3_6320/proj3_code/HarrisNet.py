...
class ChannelProductLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        I_xx = torch.mul(x[:,0,:,:], x[:,0,:,:]).unsqueeze(0)
        I_yy = torch.mul(x[:,1,:,:], x[:,1,:,:]).unsqueeze(0)
        I_xy = torch.mul(x[:,0,:,:], x[:,1,:,:]).unsqueeze(0)
...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def __init__(self, ksize: torch.Tensor = 7, sigma: torch.Tensor = 5):
        ...
        self.conv2d = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=self.ksize,
            bias=False, padding=int((self.ksize-1)/2), padding_mode='zeros', groups=3)
        gkernel = get_gaussian_kernel(self.ksize, self.sigma).unsqueeze(0).unsqueeze(0)
        gkernel = nn.Parameter(torch.cat((gkernel, gkernel, gkernel), dim=0).float())
        self.conv2d.weight = nn.Parameter(gkernel.float())
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #######################################################################
        s = x[:,0:1,:,:] + x[:,1:2,:,:]
        output = torch.mul(x[:,0:1,:,:], x[:,1:2,:,:]) - torch.mul(x[:,2:,:,:], x[:,2:,:,:]) - self.alpha * torch.mul(s, s)
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #######################################################################
        n, c, h, w = x.shape
        ksize = 7 if min(h, w) > 7 else min(h,w)
        stride = 1
        ...
        for idx in range(x.shape[0]):
            ...
            xi[xi < torch.median(xi)] = 0
            
            # ### Run a window through it and set non max to 0
            # for i in range(0, w, stride):
            #     for j in range(0, h, stride):
            #         window = xi[:,j:j+ksize,i:i+ksize]
            #         xi[:,j:j+ksize,i:i+ksize][window != torch.max(window)] = 0
            
            output[idx,:,:,:] = xi

        ### Or just use maxpool
        
        pool = nn.MaxPool2d(7, padding=3, stride = 1)(output)
        output = torch.where(output == pool, output, torch.zeros(x.shape))
...
def get_interest_points(image: torch.Tensor, num_points: int = 4500) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    # points.
    idx = torch.nonzero(R)
    confidences = R[idx[:,0], idx[:,1]]
    
    ### removes the border value - for SIFT
    x, y, confidences = remove_border_vals(image, idx[:,1], idx[:,0], confidences)
    
    idx_ = torch.argsort(confidences, descending=True)[:num_points]
    x = x[idx_]
    y = y[idx_]
    confidences = confidences[idx_]
...
def remove_border_vals(img, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    idx_h = torch.where((y > 8) & (y < (h-8)))[0].flatten()
    idx_w = torch.where((x > 8) & (x < (w-8)))[0].flatten()
    
    idx, counts = torch.cat([idx_h, idx_w]).unique(return_counts=True)
    idx = idx[torch.where(counts.gt(1))].flatten()
