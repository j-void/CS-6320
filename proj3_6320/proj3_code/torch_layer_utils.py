...
def get_sobel_xy_parameters() -> torch.nn.Parameter:
    ...
    sobel_x = torch.tensor(sobel_x).unsqueeze(0).unsqueeze(0).float()
    sobel_y = torch.tensor(sobel_y).unsqueeze(0).unsqueeze(0).float()
    kernel = nn.Parameter(torch.cat((sobel_x, sobel_y), dim=0))
    ### END OF STUDENT CODE ####
    ############################
