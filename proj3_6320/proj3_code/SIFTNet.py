...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        b, c, m, n = cosines.shape ## batch, channels, height, width
        max_cntrb = torch.zeros_like(cosines)
        grid_y, grid_x = torch.meshgrid(torch.arange(m), torch.arange(n))
        max_cntrb[torch.arange(b), cosines.argmax(dim=1).flatten(), grid_y.flatten(), grid_x.flatten()] = 1.
        im_grads_mag = torch.sqrt(im_grads[:,0,:,:]**2 + im_grads[:,1,:,:]**2)
        per_px_histogram = torch.mul(max_cntrb, im_grads_mag)
...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.layer.weight = nn.Parameter(torch.ones(8,1,4,4).float())
...
def angles_to_vectors_2d_pytorch(angles: torch.Tensor) -> torch.Tensor:
    ...
    ###########################################################################
    N = angles.shape[0]
    ax = torch.cos(angles).reshape(N,1)
    ay = torch.sin(angles).reshape(N,1)
    angle_vectors = torch.cat((ax, ay), dim=1)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        angles = torch.tensor([np.pi*1/8, np.pi*3/8, np.pi*5/8, np.pi*7/8, np.pi*9/8, np.pi*11/8, np.pi*13/8, np.pi*15/8]).float()
        vectors = angles_to_vectors_2d_pytorch(angles)
        weight_param = torch.cat((vectors, torch.tensor([[1.0, 0.]]), torch.tensor([[0., 1.0]])), dim=0)
        weight_param = weight_param.reshape(10,2,1,1)
...
def get_sift_subgrid_coords(x_center: int, y_center: int):
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    ## Get the centers of each 4x4 subgrid 
    
    grid_x, grid_y = np.meshgrid(np.arange(x_center-6, x_center+7, 4), np.arange(y_center-6, y_center+7, 4))
    x_grid, y_grid = grid_x.flatten(), grid_y.flatten()
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    b, c, h, w = img_bw.shape
    sift_output = net(img_bw)
    for i in range(x.shape[0]):
        x_grid, y_grid = get_sift_subgrid_coords(x[i], y[i])
        
        # ## Skip the feature - Taken care of border features in harris corner while detecting corners
        # if (x_grid > w).any() or (x_grid < 0).any()  or (y_grid > h).any()  or (y_grid < 0).any() :
        #     continue
                
        fvec = sift_output[:,:, y_grid, x_grid].squeeze().transpose(0,1).detach().flatten().numpy()
